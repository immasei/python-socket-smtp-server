...
def none(req:dict, content:str, email:dict, config: dict):
    if content == '.':
        req['data'] = False   
        email = process_email_content(email)

        if req['auth'] == True:
            path = os.path.join(config['inbox_path'], f'auth.{email["name"]}')
        else:
            path = os.path.join(config['inbox_path'], f'{email["name"]}')
...
def send(c: socket.socket, content:str):
    x = content + '\r\n'
    try:    
        c.send(to_byte(x))
    except Exception:
...
def recv(c: socket.socket, b: int):
    try:
        response = c.recv(b)
    except Exception:
        print('S: Connection lost\r\n', end='', flush=True)
...
def main():
    ...
    try:
        ...
        while True:  
            ...
            req = {}
            req = default(req)
            start = send(c, '220 Service ready')
            if start == False:
                continue 
            ...
            while True:
                ...
                if cmds == 'EHLO':
                    ...
                    sending = none(req, msg, email, dat)
                elif cmds == 'QUIT':
        ...
        try: 
            send(c, '421 Service not available, closing transmission channel')
        except Exception:
            sys.exit(0)
        sys.exit(0)
